...
Notification_RE.OnClientEvent:Connect(function(message)
    ...
    -- NEW: Start cooking when the pot is full
    if lowerMsg == "the cooking pot is full!" then
        if autoCookEnabled and not cookingPaused then
            CookingPotService_RE:FireServer("CookBest")
        end

    elseif lowerMsg:find("your") and lowerMsg:find("done cooking") then
...
-- Auto Cook Loop (still here to fill the pot with apples)
task.spawn(function()
